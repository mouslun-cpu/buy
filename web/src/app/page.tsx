  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        {visibleTenders.length === 0 ? <div className="empty"><strong>還沒有可顯示的標案</strong><p>下一次 Worker 推播時，案件會自動寫入這裡。</p></div> : <div className="table-wrap"><table><thead><tr><th>標案</th><th>公告／截止</th><th>預算</th><th>狀態</th><th></th></tr></thead><tbody>{visibleTenders.map((tender) => <tr key={tender.id} className="tender-row" tabIndex={0} onClick={() => { setSelected(tender); setNote(tender.statusNote ?? ""); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelected(tender); setNote(tender.statusNote ?? ""); } }}><td><strong>{tender.title}</strong><span>{tender.orgName} · {tender.jobNumber}</span></td><td><span>{tender.publishedAt || "—"}</span><small>截止 {tender.deadline || "未提供"}</small></td><td><strong>{formatMoney(tender.budget)}</strong><small>{tender.tenderWay}</small></td><td><span className={`status status-${tender.status}`}>{statusLabels[tender.status]}</span></td><td><button className="delete-icon" onClick={(event) => { event.stopPropagation(); deleteTender(tender); }} aria-label={`刪除${tender.title}`} title="刪除標案"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M10 11v6M14 11v6M9 7l1-3h4l1 3M6 7l1 13h10l1-13" /></svg></button></td></tr>)}</tbody></table></div>}
      </section>
